...
module.exports={
    register: async (req, res)=>{
        ...
        session.user={id: createdUser[0].user_id, user_name: createdUser[0].user_name}
    ...
    login: async (req, res)=>{
        ...
        const authenticated= bcrypt.compareSync(user_password, userFound[0].user_password)

        if(authenticated){
            session.user={id: userFound[0].user_id, user_name: userFound[0].user_name}
            res.status(200).send(session.user)
        }
        else{
